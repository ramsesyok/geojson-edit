...
- **選択中の編集はすべて下書き**。プロパティ・座標の入力に加え、**地図上でのドラッグ/頂点編集も**、「更新」ボタンを押すまで**ファイルに保存されない**(未確定時は「未確定の変更」表示)。
  - **「更新」**で下書き(地図＋パネル)をまとめて確定・保存。**「取消」/Esc/空白クリック/別地物の選択/ツール切替**で下書きを破棄し、選択時の状態へ復元。
...
- **座標(緯度経度)の表示・編集**: パネル上部の「座標」で選択地物の座標を編集できる。
  ...
  - **地図→パネルはライブ表示**: 地図でドラッグ/頂点編集すると値が逐次追従(保存は「更新」時)。
  - **パネル→地図は「更新」時のみ**: 値を打ち替え、「更新」で確定。位置の微調整に向く。
