...
- **name / color(組み込み)**: 設定が無くても常にパネルに表示。`name` は地図上にラベル表示(編集中も表示)、`color` はカラーピッカーで HEX 着色(点=マーカー/線=線色/面・円=枠線＋半透明の塗り)。**編集中も実際の色で表示し即時反映**(編集中は輪郭が点線)。「クリア」で既定色。
...
## 使い方(Phase 2)
- **左パネル**で操作:
  - **新規Feature追加**: ボタン→メニューから 点 / 線 / 面 / 円 を選んで地図上に作図(線・面はダブルクリックで確定)。1つ作図すると**自動選択され編集**に移る。
  - **Feature 一覧**: 行(または ✏️)クリックで選択し**その地物へパン/ズーム**。**🗑** で削除(`Ctrl+Z` で復元)。
- 選択中の編集: 頂点ドラッグ / **Shift+ドラッグで平行移動**(全図形) / 円のふちでリサイズ / **Alt+クリックで頂点削除**(線2点・面リング3点を下限)。
- 削除: 選択して **Delete / Backspace** キー、または一覧の 🗑。
- 編集の解除は、空白をクリック または **Esc キー**。
