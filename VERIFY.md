...
- **選択中の編集はすべて下書き**。地図上でのドラッグ/頂点編集、パネルの座標・プロパティ入力のいずれも、**「更新」ボタンを押すまでファイルに保存されない**(未確定時は「未確定の変更」表示)。
  - **「更新」**で下書きをまとめて確定・保存。**「取消」/Esc/空白クリック/別地物の選択/ツール切替**で下書きを破棄し、選択時の状態へ復元。
  - 空欄のプロパティはキー削除。「フィールド設定を開く」またはコマンド `GeoJSON Edit: フィールド設定を開く` で設定画面へ。
- **座標(緯度経度)の表示・編集**: パネル上部の「座標」で選択地物の座標を編集できる。
  - 点=緯度経度、円=中心の緯度経度＋半径(m)、線・面=頂点の表(各行を編集、＋で頂点追加/×で削除。線は2点・面のリングは3点を下限に維持)。入力欄は**緯度/経度の順**。
  - 座標の入力欄に**フォーカスすると対応する地図の頂点がハイライト**(円は中心)。
  - **パネル入力は Enter またはフォーカスアウトで地図の図形へ即反映**(保存は「更新」時)。プロパティ入力も同様に Enter/フォーカスアウトで反映(チェック/選択は即時)。
  - **地図優先**: 地図でドラッグ/頂点編集すると、パネルの値は地図の値で上書きされる。
