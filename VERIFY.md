...
- **入力は下書き**。プロパティも座標も**「更新」ボタンを押すまで地図・ファイルに反映されない**(押すまでは「未確定の変更」表示)。「取消」で下書きを破棄。
...
- **座標(緯度経度)の表示・編集**: パネル上部の「座標」で選択地物の座標を編集できる。
  ...
  - **地図→パネルはライブ表示(地図優先)**: 地図でドラッグ/頂点編集すると値が逐次追従する(未確定の座標下書きは地図の値で上書きされる)。
  - **パネル→地図は「更新」時のみ**: 値を打ち替え、「更新」で地図上の地物へ反映。位置の微調整に向く。
